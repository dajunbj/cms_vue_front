<template>
  <div class="page-container">
    ...
    <!-- 明細票 -->
    <el-card class="slip" shadow="never" v-loading="loadingDetail" :body-style="{padding:'16px 16px 8px'}">
      <div class="slip-header">
        <div class="title">給与明細</div>
        <div class="meta">
          <div><span>社員番号</span><strong>{{ header.employeeNo || '—' }}</strong></div>
          <div><span>氏名</span><strong>{{ header.employeeName || '—' }}</strong></div>
          <div><span>年月</span><strong>{{ header.yyyymm || '—' }}</strong></div>
        </div>
      </div>
      ...
      <div class="block">
        ...
        <el-table :data="attendanceRows" border style="width:100%" size="small">
          <el-table-column prop="a" label="就業日数" width="120" align="center"/>
          <el-table-column prop="b" label="出勤日数" width="120" align="center"/>
          <el-table-column prop="c" label="欠勤日数" width="120" align="center"/>
          <el-table-column prop="d" label="特別休暇日数" width="140" align="center"/>
          <el-table-column prop="e" label="有給休暇日数" width="140" align="center"/>
          <el-table-column prop="f" label="有給休暇残日数" width="160" align="center"/>
        </el-table>
        <el-table :data="attendanceTimeRows" border style="width:100%;margin-top:6px" size="small">
          <el-table-column prop="a" label="労働時間" width="140" align="center"/>
          <el-table-column prop="b" label="残業時間" width="140" align="center"/>
          <el-table-column prop="c" label="深夜時間" width="140" align="center"/>
          <el-table-column prop="d" label="休日労働時間" width="160" align="center"/>
          <el-table-column prop="e" label="遅刻早退時間" width="160" align="center"/>
        </el-table>
      </div>
      ...
      <div class="block">
        ...
        <el-table :data="payRows" border style="width:100%" size="small">
          <el-table-column prop="label" label="" width="220"/>
      ...
      <div class="block">
        ...
        <el-table :data="dedRows" border style="width:100%" size="small">
          <el-table-column prop="label" label="" width="220"/>
          <el-table-column prop="value" label="" align="right">
            <template #default="{row}">
              <span>{{ formatYen(row.value) }}</span>
      ...
      <div class="summary">
        <div class="cell">
          <div class="k">総支給額</div>
          <div class="v">{{ formatYen(totals.total_payment) }}</div>
        </div>
        <div class="cell">
          <div class="k">総控除額</div>
          <div class="v">{{ formatYen(totals.total_deduction) }}</div>
        </div>
        <div class="cell strong">
          <div class="k">差引支給額</div>
          <div class="v">{{ formatYen(totals.net_payment) }}</div>
        </div>
      </div>
    </el-card>
  </div>
...
const header = computed(() => ({
  ...
  yyyymm: detail.value?.salary_month?.substring(0, 7)?.replace('-', '年') + '月'
}))
...
const totals = computed(() => ({
  ...
  net_payment:     detail.value?.net_payment     || (
